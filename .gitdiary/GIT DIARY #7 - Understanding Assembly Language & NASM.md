...
Assembly language is **hardware dependent**, with a different assembly language for each type of processor. In particular, assembly language instructions can make reference to specific *registers* in the processor,

include all of the *opcodes* of the processor, and reflect the bit length of the various registers of the processor and operands of the machine language.

An assembly language or compiler programmer must therefore understand the computer’s architecture.
...
This is called Assembly Language. Programs written in assembly language (assembly programs) are translated into machine language by an **Assembler**. This program must not only do the symbolic translation, but also assign some form of memory addresses to the symbolic addresses.
...
Assembly code consists of lots of ASCII characters; this would be stored in a file called **source code**. This then forms the input to a assembler which translates the source code into machine code or *object code*.
...
Now let’s dive into the main elements of an Assembly program and in the next part we’ll write our first code in Assembly 😎.
...
If a label is present, the assembler defines the label as equivalent to the address into which the first byte of the object code generated for that instruction will be loaded. The programmer may subsequently use the label as an address or as data in another instruction’s address field.

The assembler replaces the label with the assigned value when creating an object program. Labels are most frequently used in branch instructions. Here’s an example:
...
1. The data section — Used for declaring initialized data or constants. This data does not change at runtime.
...
3. The text section — used for keep actual code. This section begins with declaration global _start, which tells the kernel where the program execution begins.
...
There are 6 registers that store the arguments of the system call used. These are the **EBX, ECX, EDX, ESI, EDI, and EBP**. These registers take the consecutive arguments, starting with the EBX register. If there are more than six arguments, then the memory location of the first argument is stored in the EBX register.
...
■ **BITS**: Specifies whether NASM should generate code designed to run on a processor operating in 16-bit mode, 32-bit mode, or 64-bit mode. The syntax is BITS XX, where XX is 16, 32, or 64. Normally, this is set by default by the operating system.

■ **DEFAULT**: Can change some assembler defaults, such as whether to use relative or absolute addressing.

■ **SECTION** or **SEGMENT**: Changes that section of the output file the source code will be assembled into. This is discussed subsequently.

■ **EXTERN**: used to declare a symbol which is not defined anywhere in the

module being assembled, but is assumed to be defined in some other module and needs to be referred to by this one.

■ **GLOBAL**: GLOBAL is the other end of EXTERN: if one module declares
...
■ **COMMON**: Used to declare common variables. A common variable is much like a global variable declared in the uninitialized data section. The difference is that if more than one module defines the same common variable, then at link time those variables will be merged and will point at the same piece of memory.

■ **CPU**: Restricts assembly to those instructions that are available on the specified CPU. For example, this directive can specify 8086, 186, and so on.

■ **FLOAT**: Allows the programmer to change some of the default settings to options other than those used in IEEE 754.

# **Writing and running your first Assembly Language code**

Now we’re going to write our first Assembly program!

## **Hello World**

Let’s start with a basic “Hello World” program in assembly. This example is written in **x86 Assembly using NASM** (Netwide Assembler) syntax for Linux systems. These are the basic steps:

1. Define the Text to Print: We need to store the message “Hello World!” in memory because assembly directly accesses memory to display text.
2. Use a System Call to Print: In assembly, we don’t have high-level functions like `print`. Instead, we use **system calls**, which allow us to ask the operating system to perform tasks like writing text to the screen.
3. Exit the Program: We also need to use a system call to properly exit the program.

We can store our “Hello World” code in a `.asm` file. To do this, you can use any code editor (like Nano or Vim) in Linux. We’ll use NeoVim:

```
nvim program.asm
```

Open the terminal and run the above command. This will open the NeoVim editor with a blank file called `program.asm`. Now you can write your Assembly code in this file.
...
- We declare the message “Hello World!” in the `.data` section. This message is stored as a **string** using the `db` (define byte) instruction.
- We add `0xA` (newline) at the end to move the cursor to a new line after printing.

### **Text Section:**

- We set up the `_start` label as the entry point of our program.
- The word “global” is used when you want the linker to be able to know the address of some a label. The object file generated will contain alink to every label declared “global”. In this case, we have to declare“_start” as global since it is required for the code to be properly linked.

**The start Label:**

- The “_start” label is essential for all Assembly programs.
...
- We use the `mov` instruction to load registers with specific values for a `sys_write` system call.
- `eax = 4`: System call number for `sys_write`.
- `ebx = 1`: File descriptor for **standard output (stdout)**.
- `ecx = hello_msg`: The address of our message to display.
- `edx = 13`: The length of the message (characters plus newline).
- `int 0x80`: This instruction calls the Linux kernel to execute the `sys_write` command.
...
Now I think you know how to handle basic input/output in Assembly. But how do we run this in a Linux terminal? Here’s a step-by-step guide:
...
The assembler (NASM) has to take your `.asm` file and translate it into machine code that can be processed by the computer. This translation results in an intermediate `.o` file, which contains the binary code but is not yet executable. You can generate this file using the command using:

```
nasm -f elf64 -o program.o program.asm
```

Here, `-f elf64` specifies the format (64-bit ELF format, used in Linux), and `-o program.o` specifies the output file name. Now link it:
...
Here, `-o program` specifies the name of the output executable file. After this step, the executable `program` can be run directly from the terminal with:
...
We can automate the above process using Makefile. This is a configuration file in Linux used by the `make` build automation tool, to compile and link programs. For assembly programs, a Makefile is particularly useful for organizing and automating the assembly, linking, and running steps, making it easier to manage programs.

You can create the Makefile using nano. Here’s a Makefile that assembles, links, and runs the above program:
...
**Variables**: `ASM`, `ASM_FLAGS`, `TARGET`, and `OBJECT` make the Makefile easier to adjust for different programs or flags.

- `ASM` is the assembler (NASM in this case).
- `ASM_FLAGS` includes options for NASM, such as `f elf64` for 64-bit object files.
- `TARGET` is the name of the executable to produce.
- `OBJECT` is the object file name.

**Rules**:

- **all**: This is the default target. Running `make` will build the executable and then run it (`./program`).
- **$(OBJECT)**: This target assembles `program.asm` into `program.o`.
- **$(TARGET)**: This target links the object file to create the final executable.
- **clean**: This target removes any generated files (`program.o` and `program`) to allow a clean build.
